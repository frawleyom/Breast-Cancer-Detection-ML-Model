...
if num_features > 30 or num_features < 1:
    ...
    # Initialize and train the QDA classifier
    qda = QuadraticDiscriminantAnalysis()
    qda.fit(X_train, y_train)

    # Predict on the test set
    y_pred = qda.predict(X_test)
    y_pred_proba = qda.predict_proba(X_test)[:, 1]  # Probability estimates for the positive class

    # Evaluate the classifier
    accuracy = accuracy_score(y_test, y_pred)
    print(f"Test set accuracy using {X.shape[1]} features: {accuracy:.2f}")
